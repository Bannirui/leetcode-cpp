...
public:
    ...
    void push(int x) {
        while(!this->m_out.empty()){
            this->m_in.push(this->m_out.top());
            this->m_out.pop();
        }
        this->m_out.push(x);
        while(!this->m_in.empty()){
            this->m_out.push(this->m_in.top());
            this->m_in.pop();
        }
    }
    
    int pop() {
        if(this->m_out.empty()){return -1;}
        int ret=this->m_out.top();
        this->m_out.pop();
        return ret;
    }
    ...
    int peek() {
        if(this->m_out.empty()){return -1;}
        return this->m_out.top();
    }
    
    bool empty() {
        return this->m_out.empty();
    }
private:
    std::stack<int> m_in;
    std::stack<int> m_out;
